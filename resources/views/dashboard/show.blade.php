...
<div class="mb-3 mb-md-4 d-flex justify-content-between align-items-center">
    ...
    <small class="text-muted">{{ __('common.welcome') }}, <strong>{{ auth()->user()->name }}</strong></small>
</div>
    ...
    <div class="col-6 col-xl-3 mb-3 mb-xl-4">
        <div class="card flex-row align-items-center p-3 p-md-4">
            ...
            <div>
                ...
                <h6 class="mb-0 text-muted">{{ __('common.total_appraisals') }}</h6>
            </div>
        </div>
    </div>
    <div class="col-6 col-xl-3 mb-3 mb-xl-4">
    ...
    <div class="col-6 col-xl-3 mb-3 mb-xl-4">
        <div class="card flex-row align-items-center p-3 p-md-4">
            ...
            <div>
                ...
                <h6 class="mb-0 text-muted">{{ __('common.pending_approval') }}</h6>
            </div>
        </div>
    </div>
    <div class="col-6 col-xl-3 mb-3 mb-xl-4">
        <div class="card flex-row align-items-center p-3 p-md-4">
            ...
            <div>
                ...
                <h6 class="mb-0 text-muted">{{ __('common.finalized') }}</h6>
            </div>
        </div>
    </div>
</div>
...
<div class="row">
    ...
    <div class="col-12 col-md-4 mb-3 mb-md-4">
        <div class="card h-100">
            <div class="card-body">
                <h6 class="font-weight-bold mb-3">{{ __('nav.periods') }}</h6>
                @forelse($openPeriods as $period)
                    <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom">
                        <div>
                            <div class="font-weight-bold" style="font-size:0.85rem;">{{ $period->name }}</div>
                            <small class="text-muted">{{ $period->appraisals_count }} {{ strtolower(__('nav.appraisals')) }}</small>
                        </div>
            ...
            <div class="card-body d-flex flex-column">
                ...
                @if($gradeDistrib->count())
                    <div class="flex-grow-1 d-flex align-items-center justify-content-center" style="min-height:180px;">
                        <canvas id="chart-grade" style="max-height:180px;"></canvas>
                    </div>
                @else
                    <p class="text-muted mb-0">{{ __('common.no_data') }}</p>
                @endif
    ...
    <div class="col-12 col-md-4 mb-3 mb-md-4">
        <div class="card h-100">
            <div class="card-body">
                <h6 class="font-weight-bold mb-3">{{ __('common.actions') }}</h6>
...
{{-- Charts row 2: Status + Departemen --}}
@if($statusDistrib->count() || $deptDistrib->count())
<div class="row">
...
<div class="card mb-3 mb-md-4">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h6 class="font-weight-bold mb-0">{{ __('nav.appraisals') }}</h6>
            <a href="{{ route('appraisal.appraisals.index') }}" class="text-muted small">{{ __('common.all') }}</a>
        </div>
        <div class="table-responsive">
            <table class="table table-sm mb-0">
                <thead>
                    <tr>
                        <th class="border-top-0">{{ __('common.employee') }}</th>
                        <th class="border-top-0">{{ __('common.period') }}</th>
...
@section('scripts')
@php
$statusLabels = ['draft'=>'Draft','submitted'=>'Menunggu Persetujuan','approved_user2'=>'Menunggu Final','approved_cfo'=>'Final','rejected'=>'Dikembalikan'];
$statusColors = ['draft'=>'#94a3b8','submitted'=>'#f59e0b','approved_user2'=>'#3b82f6','approved_cfo'=>'#22c55e','rejected'=>'#ef4444'];
@endphp
<script>
(function(){
    var palette = ['#0f2a4a','#1a3f6f','#2563eb','#0ea5e9','#22c55e','#f59e0b','#ef4444','#8b5cf6'];
    ...
    new Chart(document.getElementById('chart-grade'), {
        ...
        data: {
            ...
            datasets: [{ data: {!! $gradeDistrib->values()->toJson() !!}, backgroundColor: palette, borderWidth: 2, borderColor: '#fff' }]
        },
        options: { plugins: { legend: { position: 'bottom', labels: { padding: 12, font: { size: 12 } } } }, cutout: '62%', maintainAspectRatio: true }
    });
    @endif

    @if($statusDistrib->count())
    var sLabels = [], sData = [], sColors = [];
    var sMap = @json($statusLabels);
    var scMap = @json($statusColors);
    @foreach($statusDistrib as $s => $c)
    sLabels.push(sMap['{{ $s }}'] || '{{ $s }}');
    sData.push({{ $c }});
    sColors.push(scMap['{{ $s }}'] || '#94a3b8');
    @endforeach
    new Chart(document.getElementById('chart-status'), {
        type: 'doughnut',
        data: { labels: sLabels, datasets: [{ data: sData, backgroundColor: sColors, borderWidth: 2, borderColor: '#fff' }] },
        options: { plugins: { legend: { position: 'bottom', labels: { padding: 10, font: { size: 11 } } } }, cutout: '58%', maintainAspectRatio: true }
    });
    @endif
    ...
    new Chart(document.getElementById('chart-dept'), {
        ...
        data: {
            ...
            datasets: [{ label: 'Jumlah Penilaian', data: {!! $deptDistrib->values()->toJson() !!}, backgroundColor: '#1a3f6f', borderRadius: 6, borderSkipped: false }]
        },
        options: {
            indexAxis: 'y',
