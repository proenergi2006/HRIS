...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form method="POST" action="{{ route('recruitment.onboarding.templates.store') }}" class="mb-4">
      @csrf
      <div class="form-row">
        <div class="form-group col-md-4 mb-2">
          <input type="text" name="label" class="form-control" placeholder="Nama item (mis. Serah terima laptop)" required>
        </div>
    ...
    @else
          ...
          @foreach($items as $item)
            ...
            <tr>
              ...
              <td style="width:90px;white-space:nowrap">
                ...
                <a href="#" class="btn btn-xs btn-outline-danger" data-confirm="Hapus item {{ $item->label }}?" data-confirm-title="Hapus Item" data-form="ob-del-{{ $item->id }}"><i class="gd-trash"></i></a>
              </td>
            </tr>
          @endforeach
          </tbody>
      ...
      @foreach($items as $item)
        <form id="ob-form-{{ $item->id }}" method="POST" action="{{ route('recruitment.onboarding.templates.update', $item) }}" class="d-none">@csrf @method('PUT')</form>
