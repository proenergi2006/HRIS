  ...
  <div class="card-body">
    @forelse($indTasks->sortBy('item.sort_order') as $task)
      @php
        ...
        $materialHref = $item->material_url ?: ($item->material_path
            ? route('onboarding.material', $item) . ($ext === 'pdf' ? '#view=FitH' : '')
            : null);
      @endphp
      <div class="border rounded p-3 mb-3 {{ $task->is_done ? 'bg-light' : '' }}">
        <div class="d-flex justify-content-between align-items-start">
          ...
          @if($materialHref)
            {{-- Halaman baru (tab baru), bukan modal — browser tampilkan materi penuh 1 halaman sendiri. --}}
            <a href="{{ $materialHref }}" target="_blank" rel="noopener" class="btn btn-xs btn-outline-primary">
